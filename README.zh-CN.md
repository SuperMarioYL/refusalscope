...
随时间重跑一个探针包（跨模型版本、改了探针包、或调整了护栏），并追问：*拒答范围漂移了吗？* `diff` 按 `probe_id` 对比两份 `probe --json` 快照，逐探针报告什么**新拒答**了（answer → refusal/shaping）、什么**新作答**了（refusal/shaping → answer）、以及拒答类别发生的变化——附带标签迁移与置信度差值：

```bash
refusalscope probe ... --json before.json
# …升级模型 / 修改护栏…
refusalscope probe ... --json after.json
refusalscope diff before.json after.json        # 富文本表格；加 --json 供 CI 使用
```

这是对 `probe` 已经产出的判定 JSON 的纯本地 diff——无网络、无历史存储。一旦有任何探针新拒答，退出码非零。

## 工作原理

```
trace ─▶ normalize ─▶ 8 个信号提取器 ─▶ 按标签累计证据 ─▶ 规则引擎 ─▶ Verdict
...
| `content_filter` | `hard_refusal` | provider 以 `finish_reason="content_filter"` 终止生成 |
...
- [x] **v0.2**——`diff` 对比两次运行以检测拒答范围漂移；处理结构化的 `message.refusal` / `content_filter`；更可信的置信度。
